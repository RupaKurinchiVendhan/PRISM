...
try:
    ...
    # Fallback to standard YAML loader if OrderedYaml is not available
...
def parse(opt_path, is_train=True):
    ...
    scale = 1
    # Handle distortion field - can be string (for SR) or list (for multi-degradation)
    distortion = opt.get('distortion', 'restoration')
    if distortion == 'sr':
