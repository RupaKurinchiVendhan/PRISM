...
class Raindrop():
	...
	def updateTexture(self, bg): 	
		# Use PIL ImageFilter for Gaussian blur instead of pyblur
		fg = Image.fromarray(np.uint8(bg)).filter(ImageFilter.GaussianBlur(radius=2.5))
		fg = np.asarray(fg)
	...
	def _createDefaultDrop(self):
		...
		if (self.shape == 0):    
			...
			# Use PIL ImageFilter for Gaussian blur
			blur_radius = max(1, self.blur_coeff // 2)  # Convert blur_coeff to appropriate radius
			self.alphamap = Image.fromarray(np.uint8(self.labelmap)).filter(ImageFilter.GaussianBlur(radius=blur_radius))
			self.alphamap = np.asarray(self.alphamap).astype(np.float32)
		...
		if (self.shape == 2): 
			...
			# Use PIL ImageFilter for Gaussian blur
			blur_radius = max(1, self.blur_coeff // 2)  # Convert blur_coeff to appropriate radius
			self.alphamap = img.filter(ImageFilter.GaussianBlur(radius=blur_radius))
			self.alphamap = np.asarray(self.alphamap).astype(np.float32)
			self.alphamap = self.alphamap/np.max(self.alphamap)*255.0
			# set label map
			self.labelmap[self.labelmap>0] = 1            
